...
The steps on this page only apply to files with Authenticode signatures. By default, the metainstaller is signed using a self-signed certificate. This metainstaller is named `DentalWingsUpdateSetup.exe` and can be found in the `staging` directory.

# Applying a Tag to the Metainstaller #

A tool called `ApplyTag.exe` is built as part of the normal build. You will need this tool and the metainstaller (`DentalWingsUpdateSetup.exe`) from `staging`.
...
> `ApplyTag.exe DentalWingsUpdateSetup.exe MyAppSetup.exe "appguid={C38B7539-CD23-4954-BDC8-FCE21B2543AE}&appname=My%20App&needsadmin=False&lang=en"`
